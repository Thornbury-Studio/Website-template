...
export default async function TemplatePreviewPage({ params }: PreviewPageProps) {
  ...
  return (
    <div className="min-h-screen">
      <div className="sticky top-0 z-20 border-b border-[var(--line)] bg-[color-mix(in_srgb,var(--paper)_88%,transparent)] backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1200px] flex-wrap items-center justify-between gap-3 px-5 py-3.5 sm:px-8">
          <div className="flex flex-wrap items-center gap-3">
            <Link
              href="/"
              className="rounded-full px-2.5 py-1 text-sm text-[var(--muted)] transition-colors hover:bg-white hover:text-[var(--ink)]"
            >
              ← Templates
            </Link>
            <span className="hidden h-4 w-px bg-[var(--line)] sm:block" />
            <p className="text-sm font-medium text-[var(--ink)]">{template.name}</p>
            <span className="rounded-full bg-[var(--accent-soft)] px-2.5 py-1 text-[11px] font-semibold tracking-wide text-[var(--accent-deep)] uppercase">
              {template.tier}
            </span>
          </div>
          <a
            href="/"
            className="text-xs font-medium tracking-wide text-[var(--muted)] uppercase transition-colors hover:text-[var(--ink)]"
          >
            Template Commons
          </a>
        </div>
